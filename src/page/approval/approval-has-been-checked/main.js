...
class Main extends React.Component {
    ...
    _tableColumns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                return (
                    <span key={`${id}_edit`}>
                        <a onClick={() => this._editAndCheck(OperateType, id, title, instId)}>
                            查看
                        </a>
                    </span>
                )
    ...
    _getTableData = (pageIndex, queryObject = this.state.formQueryObject) => {
        ...
        request(api.myCompletedApproval, params, 'post', session.get('token'))
            .then(res => {
                console.log('res', res);
                if (res.success) {
    ...
    _editAndCheck = (operateType, id, title, instId) => {
        const url = api.completedDetailInfoById;
                ...
                <div className='main-page'>
                    <div className='title-view'>
                        <Row type='flex' justify='space-between' align='middle'>
                            <Col>
                                <MyTitle title='我已审核' imgSrc={ass.img.titleIcon}/>
                            </Col>
                        </Row>
                    </div>
