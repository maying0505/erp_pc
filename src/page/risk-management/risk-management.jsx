...
class RiskManagementForm extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loading: true,
            tableListDataSource: [],
            pageInfo: { pageNo: '1', pageSize: '10',menuStages: JSON.stringify(["2","3","4","5"]), queryStages: "[]" },
            searchInfo: {},
                  ...
                  this.setState({
                    columns: [
                      ...
                      },
                      {
                        ...
                        render: (text, record) => text !== '1' ? <span className="green-span"  onClick={this.viewDetail.bind(this,record)}>查看</span>: <span className="green-span" onClick={this.updateDetail.bind(this,record)}>审批</span>
                      },
                    ]
                  })
                });
    ...
    updateDetail(e){ //审批
       hashHistory.push(`/risk-management/risk-management-detail/${e.customerBy.id}/${e.id}/${e.audit.id ? e.audit.id : null}/${e.audit.state ? e.audit.state : null}`)
    }
    viewDetail(e){ //查看
       hashHistory.push(`/risk-management/risk-management-detail/${e.customerBy.id}/${e.id}/${e.audit.id ? e.audit.id : null}/${e.audit.state ? e.audit.state : null}`)
    }
...
const RiskManagement = Form.create()(RiskManagementForm)
