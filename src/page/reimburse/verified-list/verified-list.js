...
class VerifeidList extends React.Component {
    ...
    _tableColumns = [
        ...
        },
        {
            ...
            dataIndex: '',
            render: (text, record) => (
                <a onClick={() => this._onOperationVerify('verified', 'verified', record.type, record.state, record.id, record.taskId, record.procInsId)}>
                    查看
                </a>
            )
        },
    ...
    _onSearchButtonPress = () => {
        ...
        const {startDate, endDate, type, companyName, deptName, name, state} = fieldsValue;
    ...
    _getTableData = (pageIndex, queryObject = this.state.formQueryObject) => {
        ...
        request(api.verifiedList, params, 'post', session.get('token'))
            .then(res => {
                console.log('res', res);
                if (res.success) {
