...
class ReCheckedForm extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loading: true,
            tableListDataSource: [],
            pageInfo: {pageNo: '1', pageSize: '10', menuStages: JSON.stringify(["7", "8", "6.5"]), queryStages: "[]"},
            searchInfo: {},
                        ...
                        this.setState({
                            columns: [
                                ...
                                },
                                {
                                    ...
                                    render: (text, record) => text !== '1' ? <span className="green-span"
                                                                                   onClick={this.viewDetail.bind(this, record)}>查看</span> :
                                        <span className="green-span"
                                              onClick={this.updateDetail.bind(this, record)}>复核</span>
                                },
                            ]
                        })
                    });
    ...
    updateDetail(e) { //审批
        hashHistory.push(`/re-checked/re-checked-detail/re_checked/${e.customerBy.id}/${e.id}/${e.audit.id ? e.audit.id : null}/${e.audit.state ? e.audit.state : null}`)
    }
    ...
    handleSearch = (e) => { //查询请求
        ...
        this.props.form.validateFields((err, values) => {
            ...
            this.setState({
                ...
                pagination: {total: 1, pageSize: 10, current: 1},
                pageInfo: {pageNo: '1', pageSize: '10', menuStages: JSON.stringify(["7", "8"]), queryStages: "[]"},
            }, function () {
                this.loadlists({...this.state.pageInfo, ...this.state.searchInfo})
            })
        });
        ...
        return (
            <div className="table-list">
                <Card bordered={false}>
                    ...
                    <Table bordered rowKey="ident" columns={this.state.columns}
                    ...
                    <ApprovalProcess onCancelAppPro={this.onCancelAppPro.bind(this)}
                                     modalVisible={this.state.modalVisible} productId={this.state.productId}/>
                </Card>
            </div>
        )
    }
...
const ReChecked = Form.create()(ReCheckedForm)
export default ReChecked
