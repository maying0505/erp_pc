...
class ReCheckedOpinionForm extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            isSave: true,
    ...
    auditInfoSave = () => {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            const url = this.props.type === 'finance' ? api.financeCheckSave : api.reCheckedSave;
            request(url, {
                ...values,
                id: this.state.auditId,
            ...
            <Spin spinning={this.state.loading} size="large">
                ...
                >
                    ...
                    <ApprovalProcess ifModal={false} modalTableColumns={this.state.modalTableColumns}
                                     apiUrl={api.groupAuditHistory} productId={this.props.productId}/>
                    <FormItem label="复核意见：">
                        {getFieldDecorator('auditOpinion', {initialValue: this.state.auditOpinion})(
                            <TextArea placeholder={
                                `1、XXX分公司（或总公司）复核人员同意（或拒接）XXX客户的借款申请；
            2、在复核过程中需要注意以下事项（或者是拒绝的原因是）：
               1）................
               2）...............
            ................`
                    ...
                    <div className="col-interval">
                        <FormItem label="复核结果：">
                            {getFieldDecorator('auditResult', {initialValue: this.state.auditResult})(
                                <Select placeholder='--请选择--' style={{width: '30%'}} disabled={this.props.disabled}
                                        allowClear={true}>
                    ...
                    <div className="col-interval">
                        {!this.props.disabled &&
                        <Button className="green-style" style={{float: 'left', marginLeft: '0'}}
                                onClick={this._onSaveDebounce}>保存</Button>}
                        {!this.props.disabled &&
                        <Button className="green-style" style={{float: 'left', marginLeft: '0'}}
                                onClick={this._onSubmitDebounce}>提交</Button>}
                        <Button className="default-btn" style={{float: 'left', marginLeft: '0'}}
...
const ReCheckedOpinion = Form.create()(ReCheckedOpinionForm);

export default ReCheckedOpinion
